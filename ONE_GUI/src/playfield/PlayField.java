...
public class PlayField extends JPanel {
	...
	public Coord mouseCoord = null;
	public List<DBHost> dbhosts = new ArrayList<DBHost>();
	public List<WifiHost> wifihosts = new ArrayList<WifiHost>();

	public boolean selectDBs = false;
	public DBHost hoveredHost = null;
	public List<DBHost> selectedDbHosts = new ArrayList<DBHost>();

	public List<Color> colors = new ArrayList<Color>();
	public List<List<DBHost>> groupCenters = new ArrayList<List<DBHost>>();

	/**
	 * Creates a playfield
	 * 
	 * @param w
	 *            The world that contains the actors to be drawn
	 */
	public PlayField(World w) {
		
		colors.add(new Color(1f, 0, 0, 0.3f)); // red
		colors.add(new Color(0, 1f, 0, 0.3f)); // green
		colors.add(new Color(0, 0, 1f, 0.3f)); // blue
		colors.add(new Color(0, 0, 0, 0.3f)); // black
		colors.add(new Color(1f, 1f, 1f, 0.3f)); // white
		colors.add(new Color(1f, 0.5f, 0, 0.3f)); // ???

		this.w = w;
		this.refGraphic = new ScaleReferenceGraphic();
		updateFieldSize();
		this.setBackground(bgColor);
		this.overlayGraphics = Collections
				.synchronizedList(new ArrayList<PlayFieldGraphic>());
	 ...
	 * 
	 * @param image
	 *            The image to set or null to remove the image
	 * @param dx
	 ...
	 * @param rotation
	 *            Rotatation angle of the image (radians)
	 */
	 ...
	 * 
	 * @param show
	 *            True if the map graphics should be shown (false if not)
	 */
	...
	/**
	 * Enables or disables the automatic clearing of overlay graphics. If
	 * enabled, overlay graphics are cleared every time a new graphics object is
	 * set to be drawn.
	 * 
	 * @param clear
	 *            Auto clear is enabled if this is true, disabled on false
	 */
	 ...
	 * 
	 * @param g
	 *            The graphics context to draw the field to
	 */
	public void paint(Graphics g) {
		...
		/*
		 * draw hosts for (DTNHost h : w.getHosts()) { new
		 * NodeGraphic(h).draw(g2); // TODO: Optimization..? }
		 */
		...
		if (mouseCoord != null) {
			Ellipse2D.Double circle = new Ellipse2D.Double(
					scale(mouseCoord.getX() - 5), scale(mouseCoord.getY() - 5),
					scale(10), scale(10));
			g2.setColor(Color.ORANGE);
			g2.fill(circle);
		}
		for (int i = 0, n = dbhosts.size(); i < n; i++) {
			DBHost host = dbhosts.get(i);
			host.draw(g2);
		}

		// when selecting DBs for DM
		if (selectDBs) {

			DBHost prevHost = null;
			for (int i = 0, n = selectedDbHosts.size(); i < n; i++) {
				DBHost host = selectedDbHosts.get(i);
				host.drawSelected(g2);
				g2.setFont(g2.getFont().deriveFont(Font.BOLD));
				g2.drawString(String.valueOf(i + 1),
						scale(host.centerCoord.getX() - host.diameter / 2),
						scale(host.centerCoord.getY() - host.diameter / 2 - 10));

				if (prevHost != null) {
					g2.setColor(Color.BLUE);
					g2.drawLine(scale(prevHost.centerCoord.getX()),
							scale(prevHost.centerCoord.getY()),
							scale(host.centerCoord.getX()),
							scale(host.centerCoord.getY()));
			...
			if (hoveredHost != null) {
				hoveredHost.drawHovered(g2);
			}
		}

		for (int i = 0, n = groupCenters.size(); i < n; i++) {
			Color color = colors.get(i % colors.size());
			for (DBHost host : groupCenters.get(i)) {
				host.drawColorRange(g2, color);
			}
		}
		
		for (int i = 0, n = wifihosts.size(); i < n; i++) {
			WifiHost host = wifihosts.get(i);
	 ...
	 */
	/*
	 * public void addMessageTransfer(DTNHost from, DTNHost to) { autoClear();
	...
	/**
	 * Returns a world location for a given graphical location. Note that there
	 * might be inaccuracies because of rounding.
	 * 
	 * @param loc
	 *            The location to convert
	...
	/**
	 * Updates the playfields (graphical) size to match the world's size and
	 * current scale/zoom.
	 */
	...
	public void startDBselectionforDM() {
		this.selectDBs = true;
		this.selectedDbHosts = new ArrayList<DBHost>();
	}

	public void addtoSelectedDBHosts(DBHost host) {
		this.selectedDbHosts.add(host);
	}

	public void finishDBSelectionforDM() {
		this.selectDBs = false;
	}
}
