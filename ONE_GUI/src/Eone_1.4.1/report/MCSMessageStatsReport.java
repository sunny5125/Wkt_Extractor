 ...
 */
public class MCSMessageStatsReport extends Report implements MessageListener {
	...
	protected void init() {
		...
		this.nd=0;
		try{
			this.fw = new FileWriter("N_Destination.txt");
			this.br = new BufferedWriter(fw);
			this.fw2 = new FileWriter("N_C_D.txt");
	...
	public void newMessage(Message m) {
		if(m.getId().startsWith("N")){
			try{
				br.write(m.toString()+ " "+m.getCreationTime()+" "+m.getTo().toString()+"\n");
			}catch(Exception e)
			{
			}
	...
	@Override
	public void done() {
		write("Emergency Message stats for scenario " + getScenarioName() + 
		...
		String statsText = "created: " + this.nrofCreated + 
			...
			"\nCountable Msg : "+this.nc+
			"\nDelivered Within AT :"+this.nd
			;
