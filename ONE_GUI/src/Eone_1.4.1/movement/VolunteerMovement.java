...
public class VolunteerMovement extends MapBasedMovement {
	...
	public VolunteerMovement(Settings settings) {
		super(settings);
		// this.neighborhood =
		// Integer.parseInt(settings.getSetting("neighborhood"));
		// this.radius = Double.parseDouble(settings.getSetting("radius"));
		this.radius = settings.getDouble("clusterRange");
		this.map = getMap();
		...
		if (dbMapNode.getLocation().distance(dbCoord) > radius)
	...
	private MapNode getHome() {
		...
		/*
		 * if (PDMConfig.getCenters().get("neighborhood") == null) { System.err
		 * .println("VolunteerMovement: No neighborhood type is declared.");
		 * System.exit(-1); }
		 * 
		 * try { currCenter = PDMConfig.getCenters().get("neighborhood")
		 * .get(neighborhood); } catch (ArrayIndexOutOfBoundsException aiex) {
		 * System.err.println("VolunteerMovement: Invalid neighorhood " +
		 * neighborhood); System.exit(-1); }
		 */
		
		//currCenter = rangeMapNodes.get(rng.nextInt(rangeMapNodes.size()));
		currCenter=dbMapNode;
	...
	public Path getPath() {
		...
		do {
			...
			// DisasterMovement.getCenters().get(servicingCenter).get(rng.nextInt(DisasterMovement.getCenters().get(servicingCenter).size()));
				...
				|| to.getLocation().distance(getHome().getLocation()) > radius);
