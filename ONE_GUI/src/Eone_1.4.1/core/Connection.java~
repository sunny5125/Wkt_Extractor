...
public abstract class Connection {
		...
		public void finalizeTransfer() 
	{
		...
		getOtherNode(msgFromNode).messageTransferred(this.msgOnFly.getId(),
				msgFromNode);
		if(!msgFromNode.toString().startsWith("p") && !getOtherNode(msgFromNode).toString().startsWith("p")) // clear the message from the buffer only if both the nodes arent DTN NODES
		{
			msgFromNode.deleteMessage(msgOnFly.getId(),false);
		}
		else if(msgFromNode.toString().startsWith("p") && !getOtherNode(msgFromNode).toString().startsWith("p"))
		{
			msgFromNode.deleteMessage(msgOnFly.getId(),false);
		}
